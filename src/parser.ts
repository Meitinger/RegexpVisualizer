...
export const Parser = P.createLanguage<{
  ...
  CLASSES: () => P.regexp(/\\[dDwWsS]/).map(s => {
    switch (s[1]) {
      ...
      default: throw new Error(`Class ${s} not implemented.`)
    }
  }),
  SPECIAL: () => P.regexp(/\\([trnvfb0]|u[0-9]{4})/).map(s => {
    switch (s[1]) {
      ...
      default: throw new Error(`Special character ${s} not implemented.`)
    }
  }),
  ...
  optional_number: () => P.regexp(/(0|[1-9][0-9]*)?/).map(s => s.length > 0 ? parseInt(s) : undefined),
  ...
  text: r => P.alt(r.CLASSES, r.any, r.positive_set, r.negative_set, r.char.map(char => CharSet.fromCharacters(MaxCharacter, [char]))).map(charSet => NFA.fromCharSet(charSet)),
  ...
  set_item: r => P.alt(r.CLASSES, r.range, r.set_char.map(char => CharSet.fromCharacters(MaxCharacter, [char]))),
  set_char: r => P.alt(r.SET_NON_META, r.SET_ESCAPED_META, r.SPECIAL),
  range: r => P.seq(r.set_char, r.DASH, r.set_char).map(([min, _, max]) => CharSet.fromRange(MaxCharacter, { min, max }))
})
